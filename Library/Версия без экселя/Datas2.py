...
def save_table_name():
    ...
    save_excel_type.range('A1').options(index = False).value = data_pd_type
    save_excel_orders.range('A1').options(index = False).value = data_pd_orders
    save_excel_menu.range('A1').options(index = False).value = data_pd_menu
    
    save_excel_type.autofit()
    save_excel_orders.autofit()
    save_excel_menu.autofit()
    
    wn.save() # Сохранение файла в данной директории
    wn.app.quit()
...
def export_table_name(fname):
    """
    Автор: Исмаилов Асад
    Цель: Сохранение данных датафреймов в новый файл
    Вход: Название файла Excel fname
    Выход: Новый файл с данными измененной таблицы
    
    """
    new_wb=xw.Book() # Создание нового .xlsx файла
    
    new_wb.sheets.add(name='Тип блюда')
    new_wb.sheets.add(name='Заказы')
    new_wb.sheets.add(name='Меню')
    
    export_excel_type = new_wb.sheets['Тип блюда']
    export_excel_orders = new_wb.sheets['Заказы']
    export_excel_menu = new_wb.sheets['Меню']
    
    export_excel_type.range('A1').options(index = False).value = data_pd_type
    export_excel_orders.range('A1').options(index = False).value = data_pd_orders
    export_excel_menu.range('A1').options(index = False).value = data_pd_menu
    
    export_excel_type.autofit()
    export_excel_orders.autofit()
    export_excel_menu.autofit()
...
def deleting_string(s, id_tree):
    ...
    if id_tree == 0: # Если выделена строка из таблицы типов
        for i in range(len(data_pd_type)-1, -1, -1):
            if int(s)+1 == int(data_pd_type.iloc[len(data_pd_type)-i-1, 0]):
                data_pd_type.drop([len(data_pd_type)-i-1], axis=0, inplace=True) # Удаление типа из датафрейма типов
    ...
    if id_tree == 1: # Если выделена строка из таблицы заказов
        for i in range(len(data_pd_orders)-1, -1, -1):
            if int(s)+1 == int((data_pd_orders.iloc[len(data_pd_orders)-i-1, 0])[1:]):
                data_pd_orders.drop([len(data_pd_orders)-i-1], axis=0, inplace=True) # Удаление заказа из датафрейма заказов
                # for j in range(len(data_pd_full_sub)-1, -1, -1):
                #     if int(s)+1 == int((data_pd_full_sub.iloc[len(data_pd_full_sub)-j-1, 5])[1:]):
                #         data_pd_full_sub.drop([len(data_pd_orders)-j-1], axis=0, inplace=True)
    ...
    if id_tree == 2: # Если выделена строка из таблицы Меню
        for i in range(len(data_pd_menu)-1, -1, -1):
            if int(s)+1 == int(data_pd_menu.iloc[len(data_pd_menu)-i-1, 0]):
                data_pd_menu.drop([len(data_pd_menu)-i-1], axis=0, inplace=True) # Удаление блюда из датафрейма Меню
                # data_pd_full.drop([len(data_pd_menu)-i-1], axis=0, inplace=True)
